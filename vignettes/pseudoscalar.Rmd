...
$a\cdot\left(AI\right)=a\wedge AI$ and $A\cdot\left(BI\right)=A\wedge
BI$ for vector $a$ and multivectors $A,B$:
...
B <- rcliff()
a
A  # B looks very similar
```

Above we choose randomish values for vector $a$ and multivectors
$A,B$.  Observe that $A$ and $B$ have terms of different grades; they
are not homogenous.  Numerical verification is straightforward [NB:
...
LHS <- cliffdotprod(A, B*I) # Usual idiom would be "A %.% (B*I)"
RHS <- (A^B)*I
LHS - RHS
```
