...
identify $\mathbb{H}$ with $Cl(3,0)$ [technically we do
not need to call `signature()` but it is probably defensive programming to call it anyway]
...
Alternatively we might consider the even subalgebra of $Cl(0,3)$ with
general element $q_0 + q_1e_{23} -q_2e_{13} + q_3e_{12}$ (note change
of sign for $q_2$).  Thus
