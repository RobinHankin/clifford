...
```{r}
c(drop(point(a)  %.% point(b)), -sum((a-b)^2)/2)
```
...
Then points that lie on the sphere are the set
...
For example, if our four points are $(0,0,0)$, $(1,0,0)$, $(0,1,0)$,
$(0,0,1)$ we would have

```{r}
spherestar(point(c(0,0,0)),point(c(1,0,0)),point(c(0,1,0)),point(c(0,0,1)))
point(c(0,0,0)) %^% point(c(1,0,0)) %^% point(c(0,1,0)) %^% point(c(0,0,1))
```
