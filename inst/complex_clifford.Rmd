...
We use $Cl(0,1)$, so $e_1^2=-1$.  Package idiom is straightforward; to
coerce complex numbers to Clifford objects and vice versa, we need a
couple of functions:
...
whenever $e_1^2e_1^2=1$; above we used $Cl(2,0)$ so $e_1^2=e_2^2=1$.
However, the relation is also satisfied if $e_1^2=e_2^2=-1$, so we can
equally well use $Cl(0,2)$:
...
It is best to return the signature to its default value in order to prevent
interference with other vignettes:
